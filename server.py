...
def connectionLoop(sock):
    while True:
        ...
        strData = str(data)
        # print("Got this: " + strData)

        # if addr (i.e IP,PORT) exists in clients dictionary
        if addr in clients:
            # update the heartbeat value if data dictionary has a key called 'heartbeat'
            if 'heartbeat' in strData:
                clients[addr]['lastBeat'] = datetime.now()
            else:
                # parse json data to python obj
                JSONdata = json.loads(data)
                # set the position data to client
                clients[addr]['position'] = JSONdata['position']
...
# send game states to all clients
# update client info
def gameLoop(sock):
    ...
    while True:
        ...
        for c in clients:
            ...
            # assign a random color to clients
            clients[c]['color'] = {
                "R": random.random(),
                "G": random.random(),
                "B": random.random()
